...
function et_InitGame(levelTime, randomSeed, restart)
    ...
    et.trap_FS_CreatePath(REQUESTS_DIR)
    et.G_Print("^2VPN Detection: Using requests directory: " .. REQUESTS_DIR .. "\n")
end
...
local function create_request_file(ip)
    ...
    if handle > 0 then
        ...
        local masked_url = "https://api11.scamalytics.com/v3/bahdgt/?key=" .. API_KEY:sub(1, 8) .. "...&ip=" .. ip
        et.G_Print("^2VPN Detection: Created request file: " .. filename .. "\n")
        et.G_Print("^2VPN Detection: Request URL: " .. masked_url .. "\n")
        return true
    else
...
local function list_files_in_directory(dir)
    et.G_Print("^2VPN Detection: Listing files in directory: " .. dir .. "\n")
    
    local files = {}
    local handle, dirlist
    
    handle, dirlist = et.trap_FS_GetFileList(dir, "", 1024)
    
    if handle > 0 then
        local count = et.trap_FS_GetFileListCount(handle)
        
        for i = 0, count - 1 do
            local filename = et.trap_FS_GetFileListItem(handle, i)
            et.G_Print("^2VPN Detection: Found file: " .. dir .. "/" .. filename .. "\n")
            table.insert(files, filename)
        end
        
        et.trap_FS_FreeFileList(handle)
    else
        et.G_Print("^1VPN Detection: Failed to list files in directory: " .. dir .. "\n")
    end
...
local function check_response_file(ip)
    local response_prefix = "response_" .. ip
    local filename = REQUESTS_DIR .. "/" .. response_prefix
    
    et.G_Print("^2VPN Detection: Looking for response file: " .. filename .. "\n")
    
    local files = list_files_in_directory(REQUESTS_DIR)
    
    local handle, result = et.trap_FS_FOpenFile(filename, et.FS_READ, 0)
    
    if handle <= 0 then
        for _, file in ipairs(files) do
            if file:sub(1, #response_prefix) == response_prefix then
                et.G_Print("^2VPN Detection: Found partial match: " .. REQUESTS_DIR .. "/" .. file .. "\n")
                handle, result = et.trap_FS_FOpenFile(REQUESTS_DIR .. "/" .. file, et.FS_READ, 0)
                filename = REQUESTS_DIR .. "/" .. file
                break
            end
        end
    end
    ...
    if handle > 0 then
        ...
        et.G_Print("^2VPN Detection: Found response file: " .. filename .. "\n")
        et.G_Print("^2VPN Detection: Response content: " .. content .. "\n")
        
        et.trap_FS_Rename(filename, filename .. "_processed")
        
        return content
    else
        et.G_Print("^1VPN Detection: No response file found for IP: " .. ip .. "\n")
    end
...
local function check_vpn(ip)
    ...
    create_request_file(ip)
    
    et.G_Sleep(1000)
    
    local http_result = check_response_file(ip)
    
    if not http_result then
        local masked_url = "https://api11.scamalytics.com/v3/bahdgt/?key=[YOUR_API_KEY]&ip=" .. ip
        et.G_Print("^3WARNING: To manually check this IP, visit:\n^7" .. masked_url .. "\n")
        http_result = "{}"  -- Empty JSON response as fallback
    end
